...
**Versão:** [2.0.0](https://github.com/vallades/simplepad/releases/tag/v2.0.0) · **Licença:** [MIT](./LICENSE)
...
- **File Explorer** — sidebar esquerda (⌘B), árvore lazy, busca, clique abre nota; fechada por padrão
...
- **Snippets** — `;hoje`, `;check`, etc.; **Tab** ou **Ctrl/Cmd+Espaço**; editáveis em Settings (globais)
- **Overflow de abas** — botão **…** com lista completa quando há muitas abas
- **Persistência de sessão** — por workspace (ou Pessoal); restaura abas, cursor, scroll e `isMarkdown`
- **Arquivos** — Abrir / Salvar / Salvar como + **Recentes** (máx. 10)
- **Drag & drop** de arquivos de texto (`.txt`, `.md`, `.json`, código…) → novas abas + overlay
- **Templates** — por workspace; Daily Note, Reunião, Ideia, Checklist
- **Configurações** — fonte, tema, auto-save, Markdown, Mermaid, Properties, Templates, Snippets
- **Auto-save** — arquivos no disco **e** rascunhos “Sem título” (`untitled-notes/` do workspace)
...
| [docs/PROJECT_OVERVIEW.md](./docs/PROJECT_OVERVIEW.md)         | Overview atual (**v2.0**)                            |
